...
public class Segment implements Shape {
	private String id; // identifies the clients
	...
	/**
	 * Encodes Segment instance as String so that it can be transmitted through server/client interactions
	 * @return
	 */
	@Override
	public String toString() {
		//if the id is null, id is not returned
		if (this.id == null){
			return "segment "+x1+" "+y1+" "+x2+" "+y2+" "+color.getRGB();
		}
		//else the return val includes the id as well
		else {
			return "segment "+x1+" "+y1+" "+x2+" "+y2+" "+color.getRGB() + " " + this.id;
		}
	}
